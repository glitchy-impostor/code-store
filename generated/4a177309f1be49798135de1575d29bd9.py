...
def run_code_and_driver(code, driver_function, command_args, schedule_time, recipient_email):
    ...
    job = scheduler.add_job(driver_function, 'date', run_date=run_date, args=[code, command_args], id="execute_job")
    
    def callback(x):
        driver_function(code, x, recipient_email)  # Call the driver function with the result and email

    if job:
        job.add_listener(callback, events=['completed'])
    scheduler.start()

def driver_function(code, result, recipient_email):
    """
...
def recurring_driver_function(code, driver_function, command_args, interval, recipient_email):
    ...
    scheduler = BackgroundScheduler()
    job = scheduler.add_job(driver_function, 'interval', seconds=interval, args=[code, command_args], id="execute_job")
...
"""
    ...
    run_code_and_driver(code, driver_function, command_args, schedule_time, recipient_email)
